...
public class BaseDiagramUtil {
	...
	public static void saveDPIAnnotation(final Package model) {
		ICommand command = new AbstractTransactionalCommand(TransactionUtil.getEditingDomain(model), "Add DPI", null) { //$NON-NLS-1$
	...
	public static View getContextEditPartView() {
		...
		if (selection instanceof IStructuredSelection) {
			if (((IStructuredSelection) selection).getFirstElement() instanceof IAdaptable) {
				view = ((IAdaptable) ((IStructuredSelection) selection).getFirstElement()).getAdapter(View.class);
			}
		}
	...
	public static EditPart getContextEditPart() {
		...
		if (selection instanceof IStructuredSelection) {
			if (((IStructuredSelection) selection).getFirstElement() instanceof IAdaptable) {
				return ((IAdaptable) ((IStructuredSelection) selection).getFirstElement()).getAdapter(EditPart.class);
			}
		}
		return part;
	}
	 ...
	 * @param object
	 */
	public static void dropElement(DiagramEditPart editPart, Point dropLocation, EObject object) {
		...
		org.eclipse.gef.commands.Command command = editPart.getCommand(dropRequest);
	 ...
	 */
	public static void showRelationships(DiagramEditPart editPart, EObject object, List<IElementType> typesToShow) {
		...
		ShowHideRelationshipsRequest showRelationshipRequest = new ShowHideRelationshipsRequest(selectedShapes,
				typeToShow, Collections.EMPTY_LIST);

		Command showRelationshipCommand = editPart.getCommand(showRelationshipRequest);
	...
	public static boolean startInlineEdit(EObject eObject) {
		...
		if (partView != null) {
			EditPart part = BaseDiagramUtil.findEditPartFromActiveDiagramEditPart(eObject);
			if (part != null && !(part instanceof DiagramEditPart)) {
				part.performRequest(new Request(org.eclipse.gef.RequestConstants.REQ_DIRECT_EDIT));
				return true;
			}
		}
		return false;
	 ...
	 * 
	 * @param diagramPart   Diagram edit part to search
	 * @param elementToFind Element to find
	 * @return
	 */
	public static EditPart findEditPartFromEditor(IDiagramWorkbenchPart diagramPart, EObject elementToFind) {
		if (diagramPart == null) {
		...
		if (elementToFind instanceof Connector) {
			for (Object o : diagramPart.getDiagramEditPart().getConnections()) {
				if (o instanceof ConnectionEditPart && ((ConnectionEditPart) o).getModel() instanceof View) {
					EObject eobj = ((View) ((ConnectionEditPart) o).getModel()).getElement();
			...
			part = diagramPart.getDiagramEditPart().findEditPart(diagramPart.getDiagramEditPart().getRoot(),
					elementToFind);
		}
		return part;
	}
	 ...
	 * @return
	 */
	public static EObject findElementFromDiagram(Diagram diagram, EObject element) {
