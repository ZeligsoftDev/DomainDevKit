 ...
 *******************************************************************************/
package com.zeligsoft.domain.zdl.architecture.commands;
...
import com.zeligsoft.domain.zdl.architecture.Activator;

/**
 * The Class CreateSysMLModelCommand.
 */
public class CreateZDLModelCommand implements IModelCreationCommand {
...
//		}
		...
		Profile profile = (Profile) PackageUtil.loadPackage(URI.createURI("pathmap://ZDL_PROFILES/ZDL.profile.uml"),
				owner.eResource().getResourceSet());
		if (profile != null) {
			Package model = (org.eclipse.uml2.uml.Package) owner;
			PackageUtil.applyProfile(model, profile, true);
			ZDLUtil.addZDLConcept(model, ZDLNames.DOMAIN_MODEL);
		} else {
			Activator.log.error("Impossible to find ZDL profile", null);
		}
	}
